...
function checkIfStartsWith(mainStr, subStr) {
  ...
  //   return mainStr.startsWidth(subStr);

  let a = mainStr.startsWith(subStr);
  return a;
}
...
function padString(original, length, padWith, side) {
  ...
  if (side === "start") {
    return padWith.padStart(length) + original;
  } else if (side === "end") {
    return original + padWith.padEnd(length);
  } else {
